 ...
 * Valid ~1800–2100; typical error well under 1° for travel use.
 */
...
function atmosphericRefraction(elevationDeg) {
  ...
  if (elevationDeg > -0.575) {
    return (
      (1 / 3600) *
      (1735 +
        h * (-518.2 + h * (103.4 + h * (-12.79 + h * 0.711))))
    );
  }
 ...
 * @param {Date} date UTC or local Date object (uses absolute instant)
 * @returns {{ azimuth: number, elevation: number, zenith: number }}
 */
export function getSunPosition(latDeg, lonDeg, date) {
  ...
  // const sunTrueAnom = M + C;
  // const sunRadVector =
  //   (1.000001018 * (1 - e * e)) / (1 + e * Math.cos((M + C) * DEG));
  ...
  let zenith = Math.acos(Math.min(1, Math.max(-1, cosZenith))) * RAD;
